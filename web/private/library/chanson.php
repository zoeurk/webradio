<?php
	class chanson{
		...
		public function __construct($init, $shuffle){
			if($init == true){
				$content = file_get_contents('../../private/config/config.json');
		...
		public function curart(){
			shell_exec('. ../../private/src/current_artist.src');
		}
		...
		public function random($val){
			switch($val){
				case "on":
					...
					break;
				case "off":
					shell_exec('. ../../private/src/order.src');
					break;
			}
		}
		public function createplaylist(){
			$tps = array();
			shell_exec('mpc -f "\"artist\": \"%artist%\", \"album\": \"%date% - %album%\", \"track\": \"%track%\" - \"%title%\"" playlist | sed \':redo; s/\(\"track\": \)\"\([0-9]\{1,2\}\)\"/\1\"0\2\"/;t redo\' | sort | sed -nf ../../private/sed/mpc2json.sed > ../../private/jsons/playlist.json; test -s ../../private/jsons/playlist.json || rm ../../private/jsons/playlist.json');
		}
		/**** Private ****/
		private function mylibrary(){
			shell_exec('mpc --wait --host='  . $this->password . '@' .  $this->host . ' crop;mpc -f "%artist% %date% %album% track %track% %title% file:%file%" listall  | sed ":redo;s/track \([0-9]\{1,2\}\) /track 0\1 /; t redo;" | sort | sed "s/^.*file://g" | mpc --host='  . $this->password . '@' .  $this->host . ' add;' . $this->shuf . 'mpc --wait --host='  . $this->password . '@' .  $this->host . ' del 0;');
		}
		...
		private function delete_song($string){
			shell_exec('mpc -f "%artist% %date% %album% %track% %title% pos=%position%" playlist | sed -n "s/^' . $string . '.* pos=\([0-9]*\)$/\1/p" | mpc --host='  . $this->password . '@' .  $this->host . ' del');
		}
		private function get_current($type){
			$tps = array();
			if($this->host == null || $this->password == null){
				$temps = shell_exec('mpc --wait -f "\[{\"chanson\":{\"artist\":\"%artist%\",\"album\":\"%album%\",\"track\":\"%track%\",\"title\":\"%title%\"}}," ' . $type . ' | sed -n -e "1 p" -e "2 s/^\[\([A-Za-z]*\)\].*\([0-9]\+\):\([0-9]\{2\}\)\/\([0-9]\+\):\([0-9]\{2\}\).*$/{\"info\":{\"status\":\"\1\",\"timeup\": 0,\"restant\": 0, \"temps\" :{\"m\":\"\2\",\"s\":\"\3\"},\"duree\":{\"m\":\"\4\",\"s\":\"\5\"}/p; 3 { s/^.*\(repeat\): \(on\|off\) *\(random\): \(on\|off\).*$/,\"\1\":\"\2\", \"\3\": \"\4\"}}\]/p;}"');
			}else{
		...
		private function g_next($type){
			$next = array();
			if($this->host == null || $this->password == null){
				$next_str = shell_exec('mpc --wait -f "\[{\"chanson\":{\"artist\":\"%artist%\",\"album\":\"%album%\",\"track\":%track%,\"title\":\"%title%\", \"time\":\"%time%\"}, \"info\":{\"id\": \"0\", \"ticket\": \"0\", \"status\": \"false\", \"time\": 0}}\]" ' . $type . ' | head -n 1');
			}else{
		...
		private function search($search){
			$find = shell_exec('mpc --wait -f "\[{\"chanson\":{\"artist\":\"%artist%\",\"album\":\"%album%\",\"track\":%track%,\"title\":\"%title%\", \"time\":\"%time%\"}, \"info\":{\"id\": \"0\", \"ticket\": \"0\", \"status\": \"false\", \"time\": 0}}\]" find '. $search);
			if($find == ""){
				return false;
			}
			return true;
		...
		public function getlib(){
			return $this->mpdjson('../../private/jsons/mpd.json');
		}
		public function getplaylist(){
			if(file_exists('../../private/jsons/playlist.json')){
				return $this->mpdjson('../../private/jsons/playlist.json');
			}
			return null;
		}
		...
		public function tolibrary(){
			if(file_exists('../../private/jsons/playlist.json')){
				unlink('../../private/jsons/playlist.json');
			}
			$this->mylibrary();
		}
	}
